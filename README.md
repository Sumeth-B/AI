...
### สิ่งที่แก้ในแอปแล้ว (Artifact version 167)

- แยกกรณี `claude.use()` คืนค่า `null` (หรือไม่มี `window.claude` เลย) ออกเป็น error code `not_in_viewer`
  แล้วแสดงข้อความบอกให้เปิดลิงก์ในเบราว์เซอร์ที่ล็อกอิน claude.ai แล้ว (ไม่ใช่จาก LINE / Facebook / แท็บแยก)
  ใช้กับทุกฟีเจอร์: AI (sample), Google Drive, Canva, Moda, Gmail และการดาวน์โหลดไฟล์
- เพิ่มข้อความเฉพาะสำหรับ `sampling_disabled` (บัญชี/องค์กรปิดการใช้ Claude ในหน้าแอป) และ `session_expired`
- หน้า Login: เปลี่ยนข้อกำหนด "เปิด Code execution and file creation" เป็น "เปิดลิงก์ในเบราว์เซอร์ที่ล็อกอิน claude.ai"
  และ "กด Allow เมื่อ Claude ขอสิทธิ์ครั้งแรก"
